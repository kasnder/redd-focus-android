...
`descMatch=exact|prefix|substring` (default `exact`) chooses how `desc` is compared, via `FilterRule.matchesDescription()` — the single definition all three match sites share. It exists because a content description is written to be read aloud and therefore carries live status ("Unread filter, 25, unselected", "someone's story, 3 of 27, Unseen."), so an exact match dies when a badge count changes. That is a platform property, not any one app's quirk, and the flag is available to blocking and navigation rules alike — **never special-case an app here**. An unparseable value drops the rule, like a malformed screen marker.

`descriptionMatch` is appended to `identity()` **only when it is not `EXACT`**, so every rule written before the flag existed keeps the key its saved state lives under. Any future field added to `identity()` must follow that shape — appended at the end, omitted at its default — or it silently orphans every user's enable/pause state.
...
```
package.name##viewId=…##desc=a|b##descMatch=exact|prefix|substring##path=Class[0]>Class[*]##category=…##comment=…##color=#RRGGBB##blockTouches=true
```
...
- **At most one navigation rule per app is enabled**, enforced in `ServiceConfig.setNavigationRuleEnabled`, which switches off any other rule for the same package and returns how many it displaced so the caller can say why a switch moved on its own. `AutoNavigator.ruleFor()` resolves a package to a single rule, so extras would be inert while still appearing switched on — `packagesWithSurplusRules()` logs a warning if the invariant ever breaks. It follows that navigation rules **never merge**: merging backs one switch with several rules, and several cannot be on at once.
...
- Users add their own with the element picker's **Open** button; those are stored in `custom_navigation_rules`, separate from `custom_rules`. Adds are deduplicated by `hasNavigationRuleLike` (identity, not text), because `comment` is outside `identity()`: two rules differing only in their comment share one preference key and would show as two rows driven by a single switch. `ElementPickerRuleGenerator.navigationSelector()` ranks selectors `viewId` > `desc` > anchored path and **returns null rather than fall back further**. A description carrying appended status is cut at the first comma and stored as `descMatch=prefix`; the cut is only the picker's suggestion, shown in full in the confirmation card, and what is stored is a plain prefix with no separator convention hidden in the matcher. Because a looser comparison can widen a selector onto its neighbours, the picker then calls `countDescriptionMatches` against the live screen and refuses anything naming more than one element — which is what makes `prefix` safe to offer at all. That check can only speak for the screen on display. That ordering deliberately inverts the usual structure-over-labels preference: a description that stops matching produces no click, whereas a path whose index has shifted produces the *wrong* one. Same reason `className`, root-relative `path` and `text` are refused outright — see the asymmetry below.
