...
- **Departure clearing.** Launcher window-state/topology events clear overlays immediately so they never remain over Home or Overview. Ambiguous foreign-window events (especially SystemUI) post `pendingClear` after `CLEAR_OVERLAYS_DELAY_MS` (150 ms) and re-inspect the active root first; that delay is what stops the notification shade flickering overlays.
- **Only the active window may cancel a departure.** Once the check is armed, the re-inspection of the active root is the sole thing allowed to conclude the user is still in the app — an event's own package never is, and neither `scheduleClearCheck()` re-arming nor a target-package event may push its deadline back. An app emits content and scroll events *while it is being animated away*, so the plausible-looking shortcuts (cancel on a target event, restart the timer on each foreign one) each drop the check that was about to notice the user had left, and nothing arms another. Equally, a pass that ends without reading the screen — null root, or a throw — has to hand back to the check rather than return, or the overlays it already attached are what the user is left staring at on their home screen.
